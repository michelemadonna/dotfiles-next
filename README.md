...
When Homebrew must be installed, the installer asks for the administrator
password before starting Homebrew's official installer without its additional
confirmation prompt. This also applies to non-interactive runs.
...
| ✏️ **Micro**, **Fresh Editor**, **Vim**, or **Nano** | Select one                   | Configures the requested default editor; Micro is the default. macOS installs it with the selected package manager and Ubuntu uses APT. Repository configurations are linked for the selected editor. |
